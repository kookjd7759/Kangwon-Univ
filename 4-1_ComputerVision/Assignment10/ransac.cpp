...
void CRANSAC::convert_two_point_to_line()
{
	...
	lineEquation.m = 
		pt2.x - pt1.x == 0 ? 
		std::numeric_limits<double>::infinity() : // Vertical line
		(double)(pt2.y - pt1.y) / (pt2.x - pt1.x);
...
void CRANSAC::calculate_inlier()
{
	...
	for (int i = 0; i < point_vector_list.size(); i++)
	{
		if (i != pt1Idx && i != pt2Idx)
		{
			...
			double x(point_vector_list[i].x);
			double y(point_vector_list[i].y);
			double b(lineEquation.b);
			double m(lineEquation.m);
			double distance = fabs((-(x * m) - b + y) / sqrt(m * m + 1));
