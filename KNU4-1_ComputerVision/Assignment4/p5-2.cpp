...
int main()
{
	...
	for (int y = 0; y < original_image.rows; ++y) {
		for (int x = 0; x < original_image.cols; ++x) {
			int gy = static_cast<int>(sobelYOut.at<uchar>(y, x));
			int gx = static_cast<int>(sobelXOut.at<uchar>(y, x));
			int s = static_cast<int>(std::sqrt(gx * gx + gy * gy));

			edgeStrength.at<uchar>(y, x) = cv::saturate_cast<uchar>(s);
		}
	}
